...
class KDNode<T> implements Serializable{
   ...
   protected static <T> void nnbr(KDNode<T> kd, HPoint target, HRect hr,
    ...
    public KDNode(HPoint key, T val) {
    ...
    public void storageToDB(int depth,Connection conn) {
		String sql;
    	mySql ml=new mySql();
    	double left_child_lati=OVER_RANGE;
    	double right_child_lati=OVER_RANGE;
    	double left_child_longi=OVER_RANGE;
    	double right_child_longi=OVER_RANGE;
    	int value=(Integer)v;
    	if(left!=null){
    		left_child_lati=left.k.coord[0];
    		left_child_longi=left.k.coord[1];
    	}
    	if(right!=null){
    		right_child_lati=right.k.coord[0];
    		right_child_longi=right.k.coord[1];
    	}
    	sql="insert into KDTree(father_lati,father_longi,id,left_child_lati,left_child_longi,right_child_lati,right_child_longi) values('"+k.coord[0]+"','"+k.coord[1]+"','"+value+"','"+left_child_lati+"','"+left_child_longi+"','"+right_child_lati+"','"+right_child_longi+"')";
    	ml.modifyData(conn, sql);
    	if(left!=null){
    		left.storageToDB(depth+1, conn);
    	}
    	if(right!=null){
    		right.storageToDB(depth+1, conn);
    	}
    	return ;
	}
